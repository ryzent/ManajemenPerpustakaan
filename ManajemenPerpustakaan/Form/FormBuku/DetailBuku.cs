...
namespace ManajemenPerpustakaan
{
    public partial class DetailBuku : Form
    {
        ...
        public DetailBuku(string kode_buku)
        {
            ...
            DataTable dt = utils.readToDataTable("SELECT tb_buku.kode_buku KodeBuku, judul_buku JudulBuku, penulis_buku PenulisBuku, penerbit Penerbit, tahun_terbit TahunTerbit, tb_jenisBuku.jenis_buku JenisBuku FROM tb_buku INNER JOIN tb_jenisBuku ON tb_buku.id_jenis_buku=tb_jenisBuku.id WHERE tb_buku.kode_buku='" + kode_buku + "'");
            if (dt.Rows.Count >= 0)
            {
        ...
        private void btn_simpan_Click(object sender, EventArgs e)
        {
            if (txt_kode_buku.Text != "" || txt_judul_buku.Text != "" || txt_penulis_buku.Text != "" || txt_penerbit_buku.Text != "" || txb_tahun_terbit.Text != "")
            {
                ...
                {
                    utils.update("UPDATE tb_buku SET judul_buku='" + txt_judul_buku.Text + "', penulis_buku='" + txt_penulis_buku.Text + "', penerbit='" + txt_penerbit_buku.Text + "', tahun_terbit='" + txb_tahun_terbit.Text + "' WHERE kode_buku='" + txt_kode_buku.Text + "' ");
                    this.Close();
                }
